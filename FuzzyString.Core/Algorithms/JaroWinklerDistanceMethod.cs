...
{
    public class JaroWinklerDistanceMethod : ISearchMethod
    {
        private float threshold = 0.7f;

        private static int[] ComputeJWD(String s1, String s2)
        {
            string Max, Min;
        ...
        {
            var mtp = ComputeJWD(arg1, arg2);
            var m = (float)mtp[0];

            if (m == 0)
                return 0f;

            var j = ((m / arg1.Length + m / arg2.Length + (m - mtp[1]) / m)) / 3;
